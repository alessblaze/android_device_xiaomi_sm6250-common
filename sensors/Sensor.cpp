...
SingleTapSensor::SingleTapSensor(int32_t sensorHandle, ISensorsEventCallback* callback)
    ...
    mSensorInfo.name = "Tap Sensor";
    mSensorInfo.type =
            static_cast<SensorType>(static_cast<int32_t>(SensorType::DEVICE_PRIVATE_BASE) + 100);
    mSensorInfo.typeAsString = "org.lineage.sensor.tap_gesture";
    ...
    mPollFd = open("/sys/devices/platform/soc/a94000.spi/spi_master/spi1/spi1.0/fts_wakeup_seq", O_RDONLY);
    if (mPollFd < 0) {
...
void SingleTapSensor::writeEnable(bool enable) {
    mEnableStream.open("/sys/touchpanel/double_tap");
    if (mEnableStream) {
        char currentStatus = ' ';
        char desiredStatus = enable ? '1' : '0';
        mReadStream.open("/sys/touchpanel/double_tap");
